...
import com.amazon.deequ.anomalydetection.{Anomaly, AnomalyDetectionDataPoint}
...
class HoltWintersTest extends AnyWordSpec with Matchers {
  ...
  "Additive Holt-Winters with Extended Results" should {

    val twoWeeksOfData = setupData()

    "fail if start after or equal to end" in {
      val caught = intercept[IllegalArgumentException](
        dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(twoWeeksOfData, 1 -> 1))
    ...
    "fail if no at least two cycles are available" in {
      ...
      val caught = intercept[IllegalArgumentException](
        dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(Vector.empty, fullInterval))
    ...
    "fail for negative search interval" in {
      ...
      val caught = intercept[IllegalArgumentException](
        dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(twoWeeksOfData, negativeInterval))
    ...
    "fail for too few data" in {
      ...
      val caught = intercept[IllegalArgumentException](
        dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(shortSeries, fullInterval))
    ...
    "run anomaly detection on the last data point if search interval beyond series size" in {
      val interval = 100 -> 110
      val anomalies = dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(twoWeeksOfData, interval)
      anomalies shouldBe empty
    }
    ...
    "predict no anomaly for normally distributed errors" in {
      val seriesWithOutlier = twoWeeksOfData ++ Vector(twoWeeksOfData.head)
      val anomalies =
        dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(seriesWithOutlier, 14 -> 15)
    ...
    "predict an anomaly" in {
      val seriesWithOutlier = twoWeeksOfData ++ Vector(0.0d)
      val anomalies = dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(
        seriesWithOutlier, 14 -> Int.MaxValue)
    ...
    "detect no anomalies on constant series" in {
      val series = (0 until 21).map(_ => 1.0).toVector
      val anomalies = dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(series, 14 -> Int.MaxValue)
        .filter({case (_, anom) => anom.isAnomaly})
      anomalies shouldBe empty
    }
    ...
    "detect no anomalies on exact linear trend series" in {
      val series = (0 until 48).map(_.toDouble).toVector
      val anomalies = dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(series, 36 -> Int.MaxValue)
        .filter({case (_, anom) => anom.isAnomaly})
      anomalies shouldBe empty
    }
    ...
    "detect anomalies if the training data is wrong" in {
      ...
      val anomalies = dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(series, 14 -> 21)
        .filter({case (_, anom) => anom.isAnomaly})
  ...
  private def setupData(): Vector[Double] = {
...
object HoltWintersTest {
  ...
  def dailyMetricsWithWeeklySeasonalityAnomaliesWithExtendedResults(
                                                  series: Vector[Double],
    ...
    strategy.detectWithExtendedResults(series, interval)
  }
