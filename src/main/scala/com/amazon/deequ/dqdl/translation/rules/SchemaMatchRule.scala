...
import com.amazon.deequ.dqdl.model.SchemaMatchExecutableRule
...
object SchemaMatchRule {
  def toExecutableRule(rule: DQRule): SchemaMatchExecutableRule = {
    val referenceDatasetAlias = rule.getParameters.asScala("ReferenceDatasetAlias").replaceAll("\"", "")
    ...
    SchemaMatchExecutableRule(rule, referenceDatasetAlias, assertion)
  }
}
