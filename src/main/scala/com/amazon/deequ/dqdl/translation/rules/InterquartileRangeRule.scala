...
case class InterquartileRangeRule() extends DQDLRuleConverter {
  override def convert(rule: DQRule)
    : Either[String, (Check, Seq[DeequMetricMapping])] = {
    val col = rule.getParameters.asScala("TargetColumn")
    val check = Check(CheckLevel.Error,
      java.util.UUID.randomUUID.toString)
      .hasInterquartileRange(col, assertionAsScala(rule,
        rule.getCondition.asInstanceOf[NumberBasedCondition]))
    Right(
      addWhereClause(rule, check),
      Seq(DeequMetricMapping("Column", col,
