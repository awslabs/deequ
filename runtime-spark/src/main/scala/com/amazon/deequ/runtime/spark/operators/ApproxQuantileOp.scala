...
/**
  ...
  */
case class ApproxQuantileOp(column: String, quantile: Double, relativeError: Double = 0.01)
  extends ScanShareableOperator[ApproxQuantileState, DoubleMetric] {
  ...
  override def computeMetricFrom(state: Option[ApproxQuantileState]): DoubleMetric = {
    ...
    state match {
      case Some(theState) =>
        ...
        metricFromValue(percentile, "ApproxQuantile", column)
      case _ =>
        metricFromEmpty(this, "ApproxQuantile", column)
    }
  }

  override def toFailureMetric(exception: Exception): DoubleMetric = {
    metricFromFailure(exception, "ApproxQuantile", column)
  }
