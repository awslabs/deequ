...
import com.amazon.deequ.runtime.spark.executor.NoSuchColumnException
...
class OperatorTests extends WordSpec with Matchers with SparkContextSpec with FixtureSupport {
  ...
  "Size analyzer" should {
    "compute correct metrics" in withSparkSession { sparkSession =>
      ...
      assert(SizeOp().calculate(dfMissing) == DoubleMetric(Entity.Dataset, "Size", "*",
        Success(dfMissing.count())))
      assert(SizeOp().calculate(dfFull) == DoubleMetric(Entity.Dataset, "Size", "*",
  ...
  "Completeness analyzer" should {
    "compute correct metrics" in withSparkSession { sparkSession =>
      ...
      assert(CompletenessOp("att1").calculate(dfMissing) == DoubleMetric(Entity.Column,
        "Completeness", "att1", Success(0.5)))
      assert(CompletenessOp("att2").calculate(dfMissing) == DoubleMetric(Entity.Column,
    ...
    "fail on wrong column input" in withSparkSession { sparkSession =>
      ...
      CompletenessOp("someMissingColumn").calculate(dfMissing) match {
        case metric =>
    ...
    "work with filtering" in withSparkSession { sparkSession =>
      ...
      val result = CompletenessOp("att1", Some("item IN ('1', '2')")).calculate(dfMissing)
  ...
  "Uniqueness analyzer" should {
    "compute correct metrics" in withSparkSession { sparkSession =>
      ...
      assert(UniquenessOp("att1").calculate(dfMissing) == DoubleMetric(Entity.Column, "Uniqueness",
        "att1", Success(0.0)))
      assert(UniquenessOp("att2").calculate(dfMissing) == DoubleMetric(Entity.Column, "Uniqueness",
        "att2", Success(0.0)))


      assert(UniquenessOp("att1").calculate(dfFull) == DoubleMetric(Entity.Column, "Uniqueness",
        "att1", Success(0.25)))
      assert(UniquenessOp("att2").calculate(dfFull) == DoubleMetric(Entity.Column, "Uniqueness",
        "att2", Success(0.25)))
    ...
    "compute correct metrics on multi columns" in withSparkSession { sparkSession =>
      ...
      assert(UniquenessOp("unique").calculate(dfFull) ==
        DoubleMetric(Entity.Column, "Uniqueness", "unique", Success(1.0)))
      assert(UniquenessOp("uniqueWithNulls").calculate(dfFull) ==
        DoubleMetric(Entity.Column, "Uniqueness", "uniqueWithNulls", Success(5 / 6.0)))
      assert(UniquenessOp(Seq("unique", "nonUnique")).calculate(dfFull) ==
        DoubleMetric(Entity.Mutlicolumn, "Uniqueness", "unique,nonUnique", Success(1.0)))
      assert(UniquenessOp(Seq("unique", "nonUniqueWithNulls")).calculate(dfFull) ==
        DoubleMetric(Entity.Mutlicolumn, "Uniqueness", "unique,nonUniqueWithNulls",
          Success(3 / 6.0)))
      assert(UniquenessOp(Seq("nonUnique", "onlyUniqueWithOtherNonUnique")).calculate(dfFull) ==
    ...
    "fail on wrong column input" in withSparkSession { sparkSession =>
      ...
      UniquenessOp("nonExistingColumn").calculate(dfFull) match {
        case metric =>
          assert(metric.entity == Entity.Column)
      ...
      UniquenessOp(Seq("nonExistingColumn", "unique")).calculate(dfFull) match {
        case metric =>
  ...
  "MutualInformation analyzer" should {
    "compute correct metrics " in withSparkSession { sparkSession =>
      val dfFull = getDfFull(sparkSession)
      assert(MutualInformationOp("att1", "att2").calculate(dfFull) ==
    ...
    "yields 0 for conditionally uninformative columns" in withSparkSession { sparkSession =>
      ...
      assert(MutualInformationOp("att1", "att2").calculate(df).value == Success(0.0))
    }
  ...
  "Compliance analyzer" should {
    "compute correct metrics " in withSparkSession { sparkSession =>
      ...
      assert(ComplianceOp("rule1", "att1 > 3").calculate(df) ==
        DoubleMetric(Entity.Column, "Compliance", "rule1", Success(3.0 / 6)))
      assert(ComplianceOp("rule2", "att1 > 2").calculate(df) ==
    ...
    "compute correct metrics with filtering" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      val result = ComplianceOp("rule1", "att2 = 0", Some("att1 < 4")).calculate(df)
      assert(result == DoubleMetric(Entity.Column, "Compliance", "rule1", Success(1.0)))
    }

    "fail on wrong column input" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      ComplianceOp("rule1", "attNoSuchColumn > 3").calculate(df) match {
        case metric =>
          assert(metric.entity == Entity.Column)
  ...
  "Histogram analyzer" should {
    "compute correct metrics " in withSparkSession { sparkSession =>
      ...
      val histogram = HistogramOp("att1").calculate(dfFull)
    ...
    "compute correct metrics after binning if provided" in withSparkSession { sparkSession =>
      ...
      val dfFull = getDfMissing(sparkSession)
      val histogram = HistogramOp("att1", Some(customBinner)).calculate(dfFull)
    ...
    "compute correct metrics should only get top N bins" in withSparkSession { sparkSession =>
      val dfFull = getDfMissing(sparkSession)
      val histogram = HistogramOp("att1", None, 2).calculate(dfFull)
    ...
    "fail for max detail bins > 1000" in withSparkSession { sparkSession =>
      ...
      HistogramOp("att1", binningUdf = None, maxDetailBins = 1001).calculate(df).value match {
  ...
  "Data type analyzer" should {
    ...
    "fall back to String in case no known data type matched" in withSparkSession { sparkSession =>
      ...
      DataTypeOp("att1").calculate(df).value shouldBe
    ...
    "detect integral type correctly" in withSparkSession { sparkSession =>
      ...
      DataTypeOp("att1").calculate(df).value shouldBe Success(expectedResult)
    }
    ...
    "detect fractional type correctly for negative numbers" in withSparkSession { sparkSession =>
      ...
      DataTypeOp("att2").calculate(df).value shouldBe Success(expectedResult)
    }
    ...
    "detect fractional type correctly" in withSparkSession { sparkSession =>
      ...
      val expectedResult =
        ...
        DataTypeOp("att1_float").calculate(df).value shouldBe Success(expectedResult)
    }
    ...
    "detect integral type in string column" in withSparkSession { sparkSession =>
      ...
      val expectedResult = distributionFrom(DataTypeInstances.Integral -> DistributionValue(6, 1.0))
      DataTypeOp("att1_str").calculate(df).value shouldBe Success(expectedResult)
    }
    ...
    "detect fractional for mixed fractional and integral" in withSparkSession { sparkSession =>
      ...
      DataTypeOp("att1").calculate(df).value shouldBe Success(
        distributionFrom(
    ...
    "integral for numeric and null" in withSparkSession { sparkSession =>
      ...
      DataTypeOp("uniqueWithNulls").calculate(df).value shouldBe Success(
        distributionFrom(
  ...
  "Basic statistics" should {
    ...
    "should work correctly on decimal columns" in withSparkSession { session =>
      ...
      val result = MinimumOp("num").calculate(data)
  ...
  "Count distinct analyzers" should {
    "compute approximate distinct count for numeric data" in withSparkSession { sparkSession =>
      val df = getDfWithUniqueColumns(sparkSession)
      val result = ApproxCountDistinctOp("uniqueWithNulls").calculate(df).value
    ...
    "compute exact distinct count of elements for numeric data" in withSparkSession {
      sparkSession =>
        val df = getDfWithUniqueColumns(sparkSession)
        val result = CountDistinctOp("uniqueWithNulls").calculate(df).value
        result shouldBe Success(5.0)
      }
  }
  ...
  "Approximate quantile analyzer" should {
    ...
    "approximate quantile 0.5 within acceptable error bound" in
      withSparkSession { sparkSession =>
        ...
        val result = ApproxQuantileOp("att1", 0.5).calculate(df).value.get
    ...
    "approximate quantile 0.25 within acceptable error bound" in
      withSparkSession { sparkSession =>
        ...
        val result = ApproxQuantileOp("att1", 0.25).calculate(df).value.get
    ...
    "approximate quantile 0.75 within acceptable error bound" in
      withSparkSession { sparkSession =>
        ...
        val result = ApproxQuantileOp("att1", 0.75).calculate(df).value.get
    ...
    "fail for relative error > 1.0" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      ApproxQuantileOp("att1", quantile = 0.5, relativeError = 1.1).calculate(df).value match {
    ...
    "fail for relative error < 0.0" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      ApproxQuantileOp("att1", quantile = 0.5, relativeError = -0.1).calculate(df).value match {
        case Failure(t) => t.getMessage shouldBe "Relative error parameter must " +
    ...
    "fail for quantile < 0.0" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      ApproxQuantileOp("att1", quantile = -0.1).calculate(df).value match {
    ...
    "fail for quantile > 1.0" in withSparkSession { sparkSession =>
      val df = getDfWithNumericValues(sparkSession)
      ApproxQuantileOp("att1", quantile = 1.1).calculate(df).value match {
  ...
  "Pearson correlation" should {
    "yield NaN for conditionally uninformative columns" in withSparkSession { sparkSession =>
      val df = getDfWithConditionallyUninformativeColumns(sparkSession)
      val corr = CorrelationOp("att1", "att2").calculate(df).value.get
    ...
    "yield 1.0 for maximal conditionally informative columns" in withSparkSession { sparkSession =>
      ...
      CorrelationOp("att1", "att2").calculate(df) shouldBe DoubleMetric(
        Entity.Mutlicolumn,
        "Correlation",
        "att1,att2",
        Success(1.0)
    ...
    "is commutative" in withSparkSession { sparkSession =>
      val df = getDfWithConditionallyInformativeColumns(sparkSession)
      CorrelationOp("att1", "att2").calculate(df).value shouldBe
        CorrelationOp("att2", "att1").calculate(df).value
    }
  }
  ...
  "Pattern compliance analyzer" should {
    ...
    "match doubles in nullable column" in withSparkSession { sparkSession =>
      ...
      PatternMatchOp(someColumnName, """\d\.\d""".r).calculate(df).value shouldBe Success(0.75)
    }
    ...
    "match integers in a String column" in withSparkSession { sparkSession =>
      ...
      PatternMatchOp(someColumnName, """\d""".r).calculate(df).value shouldBe Success(0.5)
    }
    ...
    "match email addresses" in withSparkSession { sparkSession =>
      ...
      PatternMatchOp(someColumnName, Patterns.EMAIL).calculate(df).value shouldBe Success(0.5)
    }
