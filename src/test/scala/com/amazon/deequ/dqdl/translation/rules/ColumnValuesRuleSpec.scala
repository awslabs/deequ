...
class ColumnValuesRuleSpec extends AnyWordSpec with Matchers {
  ...
  "ColumnValuesRule" should {
    ...
    "convert date GREATER_THAN rule" in {
      ...
      check.constraints(0).toString should include(
        "`order_date` IS NOT NULL AND to_date(`order_date`) > '2022-01-01'")
    ...
    "convert date LESS_THAN rule" in {
      ...
      val (check, _) = result.right.get
      check.constraints(0).toString should include(
        "`order_date` IS NOT NULL AND to_date(`order_date`) < '2023-12-31'")
    }
    ...
    "convert date EQUALS rule" in {
      ...
      val (check, _) = result.right.get
      check.constraints(0).toString should include(
        "`order_date` IS NOT NULL AND to_date(`order_date`) = '2022-06-15'")
    }
    ...
    "convert date IN rule" in {
      ...
      val (check, _) = result.right.get
      check.constraints(0).toString should include(
        "`order_date` IS NOT NULL AND to_date(`order_date`) IN ('2022-01-05', '2022-03-15')")
    }
    ...
    "convert date rule with column name requiring quotes" in {
      ...
      val (check, _) = result.right.get
      check.constraints(0).toString should include(
        "`Some Date` IS NOT NULL AND to_date(`Some Date`) > '2022-01-01'")
    }
    ...
    "convert date EQUALS NULL rule" in {
      ...
      result.isRight shouldBe true
      val (check, _) = result.right.get
      check.constraints(0).toString should include("`order_date` IS NULL")
    }
    ...
    "convert date NOT_EQUALS NULL rule" in {
      ...
      result.isRight shouldBe true
      val (check, _) = result.right.get
      check.constraints(0).toString should include("`order_date` IS NOT NULL")
    }
    ...
    "reject date GREATER_THAN with zero operands" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List.empty[DateExpression].asJava
      val condition = new DateBasedCondition(
      ...
      val result = ColumnValuesRule().convert(rule)
      result.isLeft shouldBe true
      result.left.get should include("requires exactly 1 date operand")
    }

    "reject date GREATER_THAN with two operands" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List[DateExpression](
        new StaticDate("2022-01-01"), new StaticDate("2023-01-01")).asJava
      val condition = new DateBasedCondition(
    ...
    "reject date GREATER_THAN with only NULL operand" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] =
        List[DateExpression](new NullDateExpression()).asJava
      val condition = new DateBasedCondition(
        "> NULL", DateBasedConditionOperator.GREATER_THAN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)
    ...
    "reject date BETWEEN with one operand" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List[DateExpression](
        new StaticDate("2022-01-01")).asJava
      val condition = new DateBasedCondition(
        "between \"2022-01-01\" and \"\"", DateBasedConditionOperator.BETWEEN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)

      val result = ColumnValuesRule().convert(rule)
      result.isLeft shouldBe true
      result.left.get should include("requires exactly 2 date operands, got 1")
    }

    "reject date IN with no operands and no null" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List.empty[DateExpression].asJava
      val condition = new DateBasedCondition(
        "in []", DateBasedConditionOperator.IN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)

      val result = ColumnValuesRule().convert(rule)
      result.isLeft shouldBe true
      result.left.get should include("requires at least one operand")
    }

    "convert date with now() dynamic expression truncates to date" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] =
        List[DateExpression](new DateExpression.CurrentDate()).asJava
      val condition = new DateBasedCondition(
        "> now()", DateBasedConditionOperator.GREATER_THAN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)
      ...
      val (check, _) = result.right.get
      val constraint = check.constraints(0).toString
      constraint should include("IS NOT NULL AND to_date(")
      constraint should fullyMatch regex """.*'\d{4}-\d{2}-\d{2}'.*"""
    }

    "reject malformed date operand" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List[DateExpression](
        new StaticDate("2021-30-40")).asJava
      val condition = new DateBasedCondition(
        "> \"2021-30-40\"", DateBasedConditionOperator.GREATER_THAN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)
    ...
    "reject malformed date in BETWEEN" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List[DateExpression](
        new StaticDate("2022-01-01"), new StaticDate("2022-13-01")).asJava
      val condition = new DateBasedCondition(
        "between \"2022-01-01\" and \"2022-13-01\"", DateBasedConditionOperator.BETWEEN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)
    ...
    "reject malformed date mixed with NULL in IN" in {
      val parameters = Map("TargetColumn" -> "order_date")
      val operands: java.util.List[DateExpression] = List[DateExpression](
        new StaticDate("2022-00-01"), new NullDateExpression()).asJava
      val condition = new DateBasedCondition(
        "in [\"2022-00-01\", NULL]", DateBasedConditionOperator.IN, operands)
      val rule = new DQRule("ColumnValues", parameters.asJava, condition)
