...
case class KurtosisRule() extends DQDLRuleConverter {
  override def convert(rule: DQRule): Either[String, (Check, Seq[DeequMetricMapping])] = {
    ...
    val check = Check(CheckLevel.Error, java.util.UUID.randomUUID.toString)
      .hasKurtosis(col, assertionAsScala(rule,
        rule.getCondition.asInstanceOf[NumberBasedCondition]))
    Right(
      addWhereClause(rule, check),
      Seq(DeequMetricMapping("Column", col, "Kurtosis",
        "Kurtosis", None, rule = rule)))
  }
}
