...
import com.amazon.deequ.runtime.spark.{DfsMetricsRepository, SparkEngine}
import com.amazon.deequ.{ComputedStatistics, SparkContextSpec}
import com.amazon.deequ.statistics._

import scala.util.{Failure, Success}

class DfsMetricsRepositoryTest extends WordSpec with SparkContextSpec with FixtureSupport {
...
//    }
  ...
  private[this] def evaluate(session: SparkSession)
    ...
    val results = SparkEngine.computeOn(data, Seq(Size(), Distinctness(Seq("item")), Completeness("att1"),
  ...
  private[this] def createRepository(sparkSession: SparkSession): MetricsRepository = {
    ...
    new DfsMetricsRepository(sparkSession, tempDir)
  }
