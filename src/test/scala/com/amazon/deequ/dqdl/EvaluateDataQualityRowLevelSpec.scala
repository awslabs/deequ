...
class EvaluateDataQualityRowLevelSpec extends AnyWordSpec with Matchers with SparkContextSpec with FixtureSupport {
  ...
  "processRows" should {
    ...
    "handle DataFreshness as skip in row-level results" in withSparkSession { spark =>
      import spark.implicits._
      ...
      val rowLevel = results(EvaluateDataQuality.ROW_LEVEL_OUTCOMES_KEY)
      val skipArrays = rowLevel.select(RowLevelResultHelper.ROW_LEVEL_SKIP)
        .collect().map(_.getSeq[String](0))

      skipArrays.foreach { skip =>
        skip should contain("DataFreshness \"date\" <= 3 days")
      }
    }
    ...
    "skip composite when nested rule has no row-level support" in withSparkSession { spark =>
      import spark.implicits._
      val today = java.time.LocalDate.now()
      val df = Seq(
        ("1", "a", today.minusDays(1).toString),
        ("2", "b", today.toString)
      ).toDF("id", "name", "date")

      val results = EvaluateDataQuality.processRows(df,
        """Rules=[(IsComplete "name") and (DataFreshness "date" <= 3 days)]""")
