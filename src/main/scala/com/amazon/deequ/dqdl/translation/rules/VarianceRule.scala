...
case class VarianceRule() extends DQDLRuleConverter {
  override def convert(rule: DQRule): Either[String, (Check, Seq[DeequMetricMapping])] = {
    ...
    val check = Check(CheckLevel.Error, java.util.UUID.randomUUID.toString)
      .hasVariance(col, assertionAsScala(rule, rule.getCondition.asInstanceOf[NumberBasedCondition]))
    Right(
      addWhereClause(rule, check),
      Seq(DeequMetricMapping("Column", col, "Variance", "Variance", None, rule = rule)))
  }
}
