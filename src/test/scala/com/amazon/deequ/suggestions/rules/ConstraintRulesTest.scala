...
class ConstraintRulesTest
  ...
  "CompleteIfCompleteRule" should {
    "be applied correctly" in {
      ...
      val complete =
        StandardColumnProfile("col1", 1.0, 100, String, false, Map.empty, None)
      val incomplete =
        StandardColumnProfile("col1", .25, 100, String, false, Map.empty, None)
    ...
    "return evaluable constraint candidates" in
      ...
      val completeInteger =
        ...
        val fakeColumnProfile =
          getFakeColumnProfileWithNameAndCompleteness("att1", 1)

        val hasMaxConstraint = HasMax().candidate(completeInteger, 1000).constraint

        val check = Check(CheckLevel.Warning, "some")
          .addConstraint(
            CompleteIfCompleteRule()
              .candidate(fakeColumnProfile, 100)
              .constraint
          )
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint = CompleteIfCompleteRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint
  ...
  "RetainCompletenessRule" should {
    ...
    "return evaluable constraint candidates" in
      withSparkSession { session =>
        ...
        val fakeColumnProfile =
          getFakeColumnProfileWithNameAndCompleteness("att1", 0.5)
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint = RetainCompletenessRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint
  ...
  "UniqueIfApproximatelyUniqueRule" should {
    "be applied correctly" in {
      ...
      val unique =
        StandardColumnProfile("col1", 1.0, 100, String, false, Map.empty, None)
      val maybeUnique =
        StandardColumnProfile("col1", 1.0, 95, String, false, Map.empty, None)
      val maybeNonUnique =
        StandardColumnProfile("col1", 1.0, 91, String, false, Map.empty, None)
      val nonUnique =
        StandardColumnProfile("col1", 1.0, 20, String, false, Map.empty, None)

      assert(UniqueIfApproximatelyUniqueRule().shouldBeApplied(unique, 100))
      assert(
        UniqueIfApproximatelyUniqueRule().shouldBeApplied(maybeUnique, 100)
      )
      assert(
        !UniqueIfApproximatelyUniqueRule().shouldBeApplied(maybeNonUnique, 100)
      )
    ...
    "return evaluable constraint candidates" in
      withSparkSession { session =>
        ...
        val fakeColumnProfile =
          getFakeColumnProfileWithNameAndApproxNumDistinctValues("item", 100)
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint = UniqueIfApproximatelyUniqueRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint
  ...
  "RetainTypeRule" should {
    "be applied correctly" in {
      ...
      val string =
        StandardColumnProfile("col1", 1.0, 100, String, true, Map.empty, None)
      val boolean =
        StandardColumnProfile("col1", 1.0, 100, Boolean, true, Map.empty, None)
      val fractional = StandardColumnProfile(
        "col1",
        1.0,
        100,
        Fractional,
      ...
      val integer =
        StandardColumnProfile("col1", 1.0, 100, Integral, true, Map.empty, None)
      val unknown =
        StandardColumnProfile("col1", 1.0, 100, Unknown, true, Map.empty, None)

      val stringNonInferred =
        StandardColumnProfile("col1", 1.0, 100, String, false, Map.empty, None)
      val booleanNonInferred =
        StandardColumnProfile("col1", 1.0, 100, Boolean, false, Map.empty, None)
      val fractionalNonInferred = StandardColumnProfile(
        "col1",
        1.0,
        100,
        Fractional,
      ...
      val integerNonInferred = StandardColumnProfile(
        "col1",
        1.0,
        100,
        Integral,
    ...
    "return evaluable constraint candidates" in
      withSparkSession { session =>
        ...
        val check = Check(CheckLevel.Warning, "some")
          .addConstraint(
            RetainTypeRule().candidate(fakeColumnProfile, 100).constraint
          )
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint =
          RetainTypeRule().candidate(fakeColumnProfile, 100).codeForConstraint
  ...
  "CategoricalRangeRule" should {
    "be applied correctly" in {
      ...
      val nonSkewedDist = Distribution(
        Map(
      ...
      val nonSkewedIntegralDist = Distribution(
        Map(
      ...
      val skewedintegralDist = Distribution(
        Map(
      ...
      val flgDist = Distribution(
        Map("0" -> DistributionValue(6, 0.6), "1" -> DistributionValue(4, 0.4)),
        2
      )
      ...
      val stringWithFlgDist = StandardColumnProfile(
        "flg",
        1.0,
        2,
        String,
      ...
      val integralWithFlgDist = StandardColumnProfile(
        "flg",
        1.0,
        2,
        DataTypeInstances.Integral,
      ...
      val stringNoDist =
        StandardColumnProfile("col1", 1.0, 95, String, false, Map.empty, None)
      val boolNoDist =
        StandardColumnProfile("col1", 1.0, 94, Boolean, false, Map.empty, None)
      val boolWithEmptyDist = StandardColumnProfile(
        "col1",
        1.0,
        20,
        Boolean,
      ...
      val integralWithSkewedDist = StandardColumnProfile(
        "col1",
        1.0,
        100,
        DataTypeInstances.Integral,
      ...
      val integralNoDist = StandardColumnProfile(
        "col1",
        1.0,
        95,
        DataTypeInstances.Integral,
      ...
      assert(
        CategoricalRangeRule().shouldBeApplied(stringWithNonSkewedDist, 100)
      )
      assert(
        CategoricalRangeRule().shouldBeApplied(integralWithNonSkewedDist, 100)
      )
      ...
      assert(
        !CategoricalRangeRule().shouldBeApplied(integralWithSkewedDist, 100)
      )
    ...
    "return evaluable constraint candidates" in
      withSparkSession { session =>
        val dfWithColumnCandidate =
          getDfWithCategoricalColumn(session, 10, Seq("'_[a_[]}!@'", "_b%%__"))
        ...
        val fakeColumnProfile = getFakeColumnProfileWithColumnNameAndHistogram(
          "categoricalColumn",
          Some(distribution)
        )
        ...
        val check = Check(CheckLevel.Warning, "some")
          .addConstraint(
            CategoricalRangeRule().candidate(fakeColumnProfile, 100).constraint
          )
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint = CategoricalRangeRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint
  ...
  "FractionalCategoricalRangeRule" should {
    "be applied correctly" in {
      ...
      // Ratio of non-unique distinct values must be > 90%
      val nonSkewedDistWithFractionalCategoricalRange = Distribution(
        Map(
      ...
      val nonSkewedDistWithActualCategoricalRange = Distribution(
        Map(
      ...
      val somewhatSkewedDist = Distribution(
        Map(
      ...
      val nonSkewedIntegralDistWithFractionalCategoricalRange = Distribution(
        Map(
      ...
      val nonSkewedIntegralDistWithActualCategoricalRange = Distribution(
        Map(
      ...
      val somewhatSkewedIntegralDist = Distribution(
        Map(
      ...
      val stringWithNonSkewedDistWithActualCategoricalRange =
        StandardColumnProfile(
          "col1",
          1.0,
          100,
      ...
      val integralWithNonSkewedDistWithFractionalCategoricalRange =
        StandardColumnProfile(
          "col1",
          1.0,
          100,
      ...
      val integralWithNonSkewedDistWithActualCategoricalRange =
        StandardColumnProfile(
          "col1",
          1.0,
          100,
      ...
      assert(
        FractionalCategoricalRangeRule()
          .shouldBeApplied(stringWithSomewhatSkewedDist, 100)
      )
      assert(
        FractionalCategoricalRangeRule().shouldBeApplied(
          stringWithNonSkewedDistWithFractionalCategoricalRange,
          100
        )
      )
      assert(
        FractionalCategoricalRangeRule()
          .shouldBeApplied(integralWithSomewhatSkewedDist, 100)
      )
      assert(
        FractionalCategoricalRangeRule().shouldBeApplied(
          integralWithNonSkewedDistWithFractionalCategoricalRange,
          100
        )
      )
      ...
      assert(
        !FractionalCategoricalRangeRule()
          .shouldBeApplied(stringWithSkewedDist, 100)
      )
      assert(
        !FractionalCategoricalRangeRule().shouldBeApplied(
          stringWithNonSkewedDistWithActualCategoricalRange,
          100
        )
      )
      assert(
        !FractionalCategoricalRangeRule().shouldBeApplied(stringNoDist, 100)
      )
      assert(!FractionalCategoricalRangeRule().shouldBeApplied(boolNoDist, 100))
      assert(
        !FractionalCategoricalRangeRule()
          .shouldBeApplied(boolWithEmptyDist, 100)
      )
      assert(
        !FractionalCategoricalRangeRule()
          .shouldBeApplied(integralWithSkewedDist, 100)
      )
      assert(
        !FractionalCategoricalRangeRule().shouldBeApplied(
          integralWithNonSkewedDistWithActualCategoricalRange,
          100
        )
      )
      assert(
        !FractionalCategoricalRangeRule().shouldBeApplied(integralNoDist, 100)
      )
    }
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForConstraint = FractionalCategoricalRangeRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint

        val expectedCodeForConstraint =
          ".isContainedIn(\"categoricalColumn\", Array(\"_b%%__\"," +
  ...
  "NonNegativeNumbersRule and PositiveNumbersRule" should {
    ...
    "return working code to add constraint to check" in
      withSparkSession { session =>
        ...
        val codeForNonNegativeConstraint = NonNegativeNumbersRule()
          .candidate(fakeColumnProfile, 100)
          .codeForConstraint
        val expectedCodeForNonNegativeConstraint = ".isNonNegative(\"item\")"
        assert(
          expectedCodeForNonNegativeConstraint == codeForNonNegativeConstraint
        )
