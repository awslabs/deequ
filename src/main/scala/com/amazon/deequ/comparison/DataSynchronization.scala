...
object DataSynchronization {
  ...
  /** @param ds1:
    ...
    * @param mergedMaps:
    *   A map that contains the columns that the customer is going to compare.
    * @return
...
//      collects all columns from both data sets used
      val colsDS1 =
        ds1.columns.filterNot(x => colKeyMap.keys.toSeq.contains(x)).sorted
      val colsDS2 =
        ds2.columns.filterNot(x => colKeyMap.values.toSeq.contains(x)).sorted
