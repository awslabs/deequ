...
//      }
    ...
    "yield correct results for basic stats" in withSparkSession { sparkSession =>
      ...
      val contextNumeric = engine.compute(SparkDataset(dfNumeric), numericAnalysis)
      ...
      val contextInformative = engine.compute(SparkDataset(dfInformative), correlationAnalysis)
      val contextUninformative = engine.compute(SparkDataset(dfUninformative), correlationAnalysis)
    ...
    "return the correct check status for anomaly detection for different analyzers" in
      withSparkSession { sparkSession =>
        evaluateWithRepository { repository =>
          ...
          val context11Rows = SparkEngine.computeOn(getDfWithNRows(sparkSession, 11), analysis)
          val context4Rows = SparkEngine.computeOn(getDfWithNRows(sparkSession, 4), analysis)
          val contextNoRows = SparkEngine.computeOn(getDfEmpty(sparkSession), analysis)
