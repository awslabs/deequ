...
import com.amazon.deequ.runtime.spark.operators.{CompletenessOp, DistinctnessOp, SizeOp, UniquenessOp}
import com.amazon.deequ.serialization.json.SimpleResultSerde
import org.apache.spark.sql.{DataFrame, SparkSession}

class OperatorContextTest extends WordSpec with Matchers with SparkContextSpec with FixtureSupport {
  ...
  "AnalyzerContext" should {
    "correctly return a DataFrame that is formatted as expected" in withSparkSession { session =>
      ...
      evaluate(session) { results =>
        ...
        val successMetricsAsDataFrame = OperatorResults.successMetricsAsDataFrame(session, results)
  ...
  private[this] def evaluate(session: SparkSession)(test: OperatorResults => Unit): Unit = {
