...
class AbsoluteChangeStrategyTest extends WordSpec with Matchers {
  ...
  "Absolute Change Strategy" should {
    ...
    "ignore max rate if none is given" in {
      ...
      val anomalyResult = strategy.detect(data).filter({case (_, anom) => anom.isAnomaly})
      val expectedAnomalyThreshold = AnomalyThreshold(lowerBound = Bound(-1.0))
    ...
    "derive first order correctly" in {
      ...
      val expectedResult = Array(1.0, 2.0, -3.0, 1.0, 6.0)
    ...
    "derive second order correctly" in {
      ...
      val expectedResult = Array(1.0, -5.0, 4.0, 5.0)
      assert(result === expectedResult)
    }
    "derive third order correctly" in {
      ...
      val expectedResult = Array(47, 56, -280.99, 296.9765)
      assert(result === expectedResult)
    }
    ...
    "attribute indices correctly for higher orders without search interval" in {
      ...
      val expectedResult = Seq(
        (4, AnomalyDetectionDataPoint(18.0, 9.0, AnomalyThreshold(upperBound = Bound(8.0)), isAnomaly = true, 1.0)),
        (5, AnomalyDetectionDataPoint(72.0, 42.0, AnomalyThreshold(upperBound = Bound(8.0)), isAnomaly = true, 1.0))
      )
      assert(result == expectedResult)
    }
    ...
    "attribute indices correctly for higher orders with search interval" in {
      val data = Vector(0.0, 1.0, 3.0, 6.0, 18.0, 72.0)
      val strategy = AbsoluteChangeStrategy(None, Some(8.0), order = 2)
      val result = strategy.detect(data, (5, 6)).filter({case (_, anom) => anom.isAnomaly})
    ...
    "behave like the threshold strategy when order is 0" in {
      ...
      val expectedResult = Seq(
        (2, AnomalyDetectionDataPoint(4.0, 5.0, AnomalyThreshold(Bound(-2.0), Bound(2.0)), isAnomaly = true, 1.0)),
        (3, AnomalyDetectionDataPoint(-7.0, -11.0, AnomalyThreshold(Bound(-2.0), Bound(2.0)), isAnomaly = true, 1.0))
      )
      assert(result == expectedResult)
    }
