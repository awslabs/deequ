...
class StateAggregationTests extends WordSpec with Matchers with SparkContextSpec
  ...
  "State aggregation outside" should {
    ...
    "give correct results" in withSparkSession { session =>
      ...
      correctlyAggregatesStates(session, SizeOp())
      correctlyAggregatesStates(session, UniquenessOp("attribute" :: "value" :: Nil))
      correctlyAggregatesStates(session, DistinctnessOp("attribute" :: Nil))
      correctlyAggregatesStates(session, CountDistinctOp("value" :: Nil))
      correctlyAggregatesStates(session, UniqueValueRatioOp("attribute" :: "value" :: Nil))
      correctlyAggregatesStates(session, CompletenessOp("attribute"))
      correctlyAggregatesStates(session, ComplianceOp("attribute", "attribute like '%facets%'"))
      correctlyAggregatesStates(session, ApproxCountDistinctOp("attribute"))
      correctlyAggregatesStates(session, MutualInformationOp("numbersA", "numbersB"))
      correctlyAggregatesStates(session, CorrelationOp("numbersA", "numbersB"))
    }
  }
