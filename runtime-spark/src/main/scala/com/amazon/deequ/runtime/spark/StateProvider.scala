...
case class InMemorySparkStateProvider() extends SparkStateLoader with SparkStatePersister {
  ...
  override def load[S <: State[_]](analyzer: Operator[S, _]): Option[S] = {
    Option(statesByOperator.get(analyzer).asInstanceOf[S])
  }

  override def persist[S <: State[_]](analyzer: Operator[S, _], state: S): Unit = {
    statesByOperator.put(analyzer, state)
  }

  override def toString: String = {
    val buffer = new StringBuilder()
    statesByOperator.foreach { case (analyzer, state) =>
...
case class HdfsSparkStateProvider(
  ...
  private[this] def toIdentifier[S <: State[_]](analyzer: Operator[S, _]): String = {
  ...
  override def load[S <: State[_]](analyzer: Operator[S, _]): Option[S] = {
    ...
    val state: Any = analyzer match {
      ...
      case _ : FrequencyBasedOperator | _ : HistogramOp => loadDataframeLongState(identifier)
