...
case class ColumnCountRule() extends DQDLRuleConverter {
  override def convert(rule: DQRule): Either[String, (Check, Seq[DeequMetricMapping])] = {
    ...
    val check = Check(CheckLevel.Error, java.util.UUID.randomUUID.toString)
      .hasColumnCount(cc => fn(cc.toDouble))
    Right(check, Seq(DeequMetricMapping("Dataset", "*", "ColumnCount", "ColumnCount", None, rule = rule)))
  }
}
